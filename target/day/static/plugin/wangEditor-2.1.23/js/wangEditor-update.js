...
_e(function (E, $) {
    ...
    E.createMenu(function (check) {
        ...
        editor.commandHooks.lineHeight = function (value) {
            // var rangeElem = editor.getRangeElem();
            // var targetElem = editor.getSelfOrParentByName(rangeElem, 'p,h1,h2,h3,h4,h5,pre,span');
            // if (!targetElem) {
				var z = window.getSelection().getRangeAt(0);
				if(z.toString() != ''){
					var startNode = $(z.startContainer.parentElement).parents('p')[0] || z.startContainer.parentElement;
					var endNode = $(z.endContainer.parentElement).parents('p')[0] || z.endContainer.parentElement;
					var flagNode = startNode;
					var flag = true;
					var arr = [];
					arr.push($(startNode));
					...
					if(startNode != endNode){
						while(flag){
							if($(flagNode).next()[0] == $(endNode)[0] || $(flagNode).next().length == 0 || ($('.wangEditor-txt')[0].lastChild.innerHTML == '<br>' && $('.wangEditor-txt')[0].lastChild == $(flagNode).next()[0])){
								flag = false
							}else{
								arr.push($(flagNode).next())
					...
					for(var i = 0; i < arr.length; i++){
						arr[i].css('line-height', value + '');
						traversal(arr[i][0],'line-height',value+'');
					}
				}
				// return;
            // }
...
//遍历节点添加css样式
function traversal(node,styleName,styleValue){
   //对node的处理
   if(node.childNodes.length > 0){
		var i = 0, childNodes = node.childNodes,item;
	   for(; i < childNodes.length ; i++){
		 item = childNodes[i];
		 $(item).css(styleName,styleValue);
		 //if(item.nodeType === 1){
